...
def parse_args():
    parser = argparse.ArgumentParser(description="Test reward functions against stored fixtures.")
    parser.add_argument(
        "--mode",
        type=str,
        choices=["policy", "random"],
        default="policy",
        help="Which reward-test fixture to load.",
    )
...
if __name__ == '__main__':
    ...
    num_envs = data[0]['env_origins'].shape[0]
    ...
    for data_t in data:
        ...
        assert (reward_base_height.shape == (num_envs,))
        print("reward_base_height: ", reward_base_height)
        plot_data["base_height_rew"].append(reward_base_height.numpy())

        reward_base_ori = rewards.reward_track_base_orientation(data_t, cfg.rewards.terms.track_base_orientation[0],
                                                                tolerance)
        assert (reward_base_ori.shape == (num_envs,))
        ...
        assert (reward_joint_pos.shape == (num_envs,))
        ...
        assert (reward_base_vel.shape == (num_envs,))
        ...
        assert (reward_ee_pos.shape == (num_envs,))
        print("reward_ee_pos: ", reward_ee_pos)
        plot_data["ee_pos_rew"].append(reward_ee_pos.numpy())

        reward_joint_target_rate = rewards.reward_joint_targets_rate(data_t, cfg.rewards.terms.joint_targets_rate[0],
                                                                     tolerance)
        assert (reward_joint_target_rate.shape == (num_envs,))
